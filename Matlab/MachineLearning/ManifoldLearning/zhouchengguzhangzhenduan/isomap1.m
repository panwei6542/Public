...
if size(D,1)~=size(D,2)
    X = D;
    % D is the location of point in space, compute the distance matrix using some NN-graph
    % D是空间中的点的位置，使用一些NN图计算距离矩阵
...
if strcmp(distance_mode, 'local')
    ...
    Ws(Ws==Inf) = 0;
    Ws = sparse(Ws);
    if exist('dijkstra_fast')
        D = dijkstra_fast(Ws, points_list);
    else
        D = compute_distance_graph(full(Ws));
    end
    if use_landmarks
        Dfull = D;
        D = D(:,landmarks);
...
%%%%% Construct low-dimensional embeddings (Classical MDS)构建低维嵌入（Classical MDS） %%%%%
if verb
