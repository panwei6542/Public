...
% 初始化Begin initialization code - DO NOT EDIT
%保证只能运行该GUI程序的一个实例，即只能打开一个窗口。=0允许该GUI程序可以运行多个实例
gui_Singleton = 1;
%该GUI的状况，包括gui的名字、运行实例、初始化函数、输出函数、布局以及回调函数。
...
%程序在调用gui_mainfcn函数时，会给figure以及各控件加一个事件监听器（listener），用来监听用户对figure以及各控件的操作。
if nargout
...
%figure和各控件创建完毕，程序会调用OpeningFcn进行初始化，然后运行OutputFcn，
%把figure的句柄作为输出参数输出。这时，GUI界面便在屏幕上显示出来。
...
% --- Outputs from this function are returned to the command line.此功能的输出返回到命令行。
function varargout = mani_OutputFcn(hObject, eventdata, handles)
varargout{1} = handles.output;

%CreateFcn在建立这个对象控件时就触发
function MatrixEdit_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc           %判断当前matlab是不是windows版本
    set(hObject,'BackgroundColor','white');%白色
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));%默认色
end


%Callback在点击或者按下或者选中时才触发的回调函数
...
% --- Executes on button press in LoadMatrix.执行Load Matrix按钮
function LoadMatrix_Callback(hObject, eventdata, handles)
handles.X = evalin ('base', get(handles.MatrixEdit,'String'));
handles.isExample = 0;
guidata(hObject, handles);%handles和guidata，三句代码连续写，变量传递
...
Xtemp = dlmread(get(handles.FileEdit,'String'),' ');%dlmread导入矩阵
[m,n] = size(Xtemp);
% Check if last column is all zeros.  Reading error.若最后一列为零，删除最后一列
...
% --- Executes on selection change in ExampleMenu.示例菜单更改时执行
...
% --- Executes on button press in PCAButton.主成分分析
...
exampleValue = get(handles.ExampleMenu,'Value');
% Number of points to use.用到的点的数量
N = round(str2double(get(handles.PointsEdit,'String')));   
N = max(1,N);
% Second parameter for example例中的第二个参数
ExParam = str2double(get(handles.ParamEdit,'String'));  
...
% --- Plot the output embedding.绘制嵌入
...
% --- LLE ALGORITHM (using K nearest neighbors)LLE算法，K邻近点
% Written by Sam Roweis & Lawrence Saul
function [Y] = lle(X,K,d)
warning off;
[D,N] = size(X);%D是矩阵的行数，N是矩阵的列数
% STEP1: COMPUTE PAIRWISE DISTANCES & FIND NEIGHBORS 寻找邻近数据点

%if two point X=(x1,x2),Y=(y1,y2) than the distance between X and Y is sqtr((x1-y1) .^2+ (x2-y2).^2)
...
%计算neighborhood（看distance定义理解）的时候，要记住X中N代表的是点数，D代表每个点得维数，把neighborhood进行整理后会找出每个点（用X的每列表示）最近的K个列
neighborhood = index(2:(1+K),:);

% STEP2: SOLVE FOR RECONSTRUCTION WEIGHTS    计算重构权
if(K>D) 
  tol=1e-3; % regularlizer in case constrained fits are ill conditioned限制适合的规则化条件
else
  tol=0;
end
W = zeros(K,N);
...
% CALCULATION OF EMBEDDING  嵌入式计算
...
%%%%% Step 1: Construct neighborhood graph构建邻域图 %%%%%
[tmp, ind] = sort(D); 
for i=1:N
    D(i,ind((2+K):end,i)) = INF; 
end;
D = min(D,D');    %% Make sure distance matrix is symmetric确保距离矩阵是对称的
%%%%% Step 2: Compute shortest paths 计算最短路径 %%%%%
for k=1:N
     D = min(D,repmat(D(:,k),[1 N])+repmat(D(k,:),[N 1])); 
end
%%%%% Remove outliers from graph 从图中删除异常值 %%%%%
...
D = D(Y.index, Y.index); 
N = length(Y.index); 
%%%%% Step 3: Construct low-dimensional embeddings (Classical MDS) 构建低维嵌入（Classical MDS） %%%%%
opt.disp = 0; 
