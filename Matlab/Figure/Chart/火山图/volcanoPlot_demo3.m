...
fl=@(y,x)-log(y./0.05)./log(10)>1./(-0.5-x);
fr=@(y,x)-log(y./0.05)./log(10)>1./(x-0.5);


DB_up=fr(padj,logFC)&logFC>=0.5;
DB_down=fl(padj,logFC)&logFC<=-0.5;
DB_not=(~DB_down)&(~DB_up);
...
ax.YTick=0:50:300;
% ==========================================================================
DB_down_out=(fl(padj,logFC)&-log(padj)./log(10)>ax.YLim(2)&logFC<=-0.5)|logFC<ax.XLim(1);
...
DB_up_out=(fr(padj,logFC)&-log(padj)./log(10)>ax.YLim(2)&logFC>=0.5)|logFC>ax.XLim(2);
