...
% 随机生成数据
rng(5)
X=randn(100,20)+[(linspace(-1,2.5,100)').*ones(1,5),(linspace(.5,-.7,100)').*ones(1,5),...
                  (linspace(.1,-.7,100)').*ones(1,3),(linspace(.9,-.2,100)').*ones(1,3),...
                  (linspace(-.1,.7,100)').*ones(1,2),(linspace(-.9,-.2,100)').*ones(1,2)];
...
for i=1:size(Data,2)
    for j=1:size(Data,1)
        ...
        tT=mean(tY);
        if tT<0&&tT>-pi
            text(ax,tR.*cos(tT),tR.*sin(tT),num2str(Data(j,i)),'Rotation',tT./pi.*180+90,...
            'Color',1-colorFunc(Data(j,i)),'HorizontalAlignment','center')
        else
            text(ax,tR.*cos(tT),tR.*sin(tT),num2str(Data(j,i)),'Rotation',tT./pi.*180-90,...
            'Color',1-colorFunc(Data(j,i)),'HorizontalAlignment','center')
        end
