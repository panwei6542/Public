...
Za = 800+1760;                                                                   % 方位向补零数
Zr = ceil(Tr/dtau)+698;                                                         % 距离向补零数
data = cat(2,data,zeros(Na,Zr));                                            % 距离向补零
data = cat(1,zeros(800,Nr+Zr),data);
data = cat(1,data,zeros(1760,Nr+Zr));                                         % 方位向补零
Na = Na+Za;%+800=2336
Nr = Nr+Zr;%+1350=3398
...
imwrite(uint8(abs(data)),'absData.bmp','bmp');
%% 时间轴、频率轴设置
...
%clear alpha
Hrcm = exp(1i*4*pi/C*(RCMbulk'*ones(1,Nr)).*(ones(Na,1)*ftau));             % 一致RCM，式7.32，式7.22
%clear RCMbulk
...
Nta = round((800/2-1)/2);
Img = Img(Za-Nta+1:Na-Nta,Ntau+1:Ntau+Nr-Zr);                                       % 裁剪图像有效区域
Img = Img/max(max(Img));
