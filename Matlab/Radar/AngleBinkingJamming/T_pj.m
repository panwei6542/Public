...
for i = 1:k1;
    ...
    A_h_d(1) = G * sqrt((Pj(i) * Gj * Gr * G_h_d * (h)^2) / ((4 * pi)^2 * Rd(1)^2 * ls1 * ls2)); %和支路
    A_fwch_d(1) = G * sqrt((Pj(i) * Gj * Gr * G_fwch_d * (h)^2) / ((4 * pi)^2 * Rd(1)^2 * ls1 * ls2));
    ...
    for t = 0:T:20000 * T
        ...
        A_h_d(j + 1) = G * sqrt((Pj(i) * Gj * Gr * G_h_d * (h)^2) / ((4 * pi)^2 * Rd(j + 1)^2 * ls1 * ls2)); %和支路振幅
        A_fwch_d(j + 1) = G * sqrt((Pj(i) * Gj * Gr * G_fwch_d * (h)^2) / ((4 * pi)^2 * Rd(j + 1)^2 * ls1 * ls2)); %方位差支路振幅

        for m = 1:8

            if ((j + 1) >= (m - 1) * N + 1) & ((j + 1) <= (m - 1) * N + N / 2)
                A_h_d(j + 1) = 0; %和支路振幅
    ...
    JS0(i) = JS(1);
    JSd(i) = JS(dian);
    ant(i) = fwt(dian) - fh(dian);
    and(i) = fwd(dian) - fh(dian);
    Rmt(i) = Rt(dian);
    Rmd(i) = Rd(dian);
    Rdian(i) = sqrt((xm(dian))^2 + (ym(dian))^2);
    Rtl(i) = Rt(dian);
    Rtuo(i) = Rt(j + 1);
    Rdtuo(i) = Rd(j + 1);
    RR(i) = sqrt((xm(j + 1))^2 + (ym(j + 1))^2);
end

t = 10 * log10(JS0);
figure;
plot(t, Rtuo, '-k', t, Rdtuo, '-.k');
grid;
xlabel('诱饵释放时刻的J/S(dB)');
ylabel('脱靶距离和导弹到诱饵的距离(m)');
h = legend('脱靶距离', '导弹到诱饵距离');
title('诱饵释放时刻J/S与脱靶距离和导弹到诱饵的距离关系图');
